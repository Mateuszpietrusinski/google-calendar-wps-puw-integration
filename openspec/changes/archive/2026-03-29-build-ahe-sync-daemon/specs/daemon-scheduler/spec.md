...
After setup and OAuth, `python -m ahe_sync` SHALL start the APScheduler `BackgroundScheduler`, log a startup line, and block indefinitely on the main thread until a termination signal is received.

#### Scenario: Startup log line emitted
- **WHEN** the daemon starts successfully
- **THEN** it logs: `[DAEMON] Started. PUW: every N min | WPS: HH:MM, HH:MM CET`
